...
?>
	...
	<form name="form" id="form" action="index.php" method="post">
		...
		<!--<input type="hidden" name="sRoom" /> 隠さないでデータを遷移する -->
		...
		<div class="search">
			<table border="0" cellpadding="2" cellspacing="0">
				...
				<tr>
					<th>物件名</th>
					<td><input type="text" name="sArticle" value="<?php print $sArticle ?>" size="50" /></td> <!--正しい値に変更-->
					<th>キーBox番号</th>
					<td><input type="text" name="sKeyBox" value="<?php print $sKeyBox ?>" size="30" /></td>
				</tr>
				<tr>
					<th>部屋番号</th>
					<td><input type="text" name="sRoom" value="<?php print $sRoom  ?>" size="30" /></td> <!--値の位置を調整-->
					<th>3Dパース</th>
					<td><input type="text" name="sDrawing" value="<?php print $sDrawing ?>" size="30" /></td>
				</tr>
				<tr>
					<th>鍵場所</th>
					<td><input type="text" name="sKeyPlace" value="<?php print $sKeyPlace ?>" size="30" /></td> <!--正しい値に変更-->
					<th>営業担当者</th>
		...
		<div class="list">
			<table border="0" cellpadding="5" cellspacing="1">
				...
				while ($row = mysqli_fetch_array($res)) {
					$articleNo   = htmlspecialchars($row["ARTICLENO"]); //HTMLの特殊文字をエスケープする設定
