...
function fnSqlArticleList($flg, $sDel, $sArticle, $sRoom, $sKeyPlace, $sArticleNote, $sKeyBox, $sDrawing, $sSellCharge, $sPage, $orderBy, $orderTo)
{
	switch ($flg) {
		case 0:
			...
			break;
		case 1:
			$sql  = "SELECT ARTICLENO, ARTICLE, ROOM, KEYPLACE, ARTICLENOTE, KEYBOX, DRAWING, SELLCHARGE"; //スペルチェック
			break;
	}
	$sql .= " FROM TBLARTICLE";
	$sql .= " WHERE DEL = $sDel";
	if ($sArticle) {
		...
		$sql .= " AND ARTICLE LIKE '%$sArticle%'"; //AND検索にし、エスケープされた文字を元に戻す設定
	}
	if ($sRoom) {
	...
	if ($sKeyBox) {
		...
		$sql .= " AND KEYBOX LIKE '%$sKeyBox%'"; //スペルチェック
	}
	if ($sDrawing) {
	...
	if ($flg) {
		$sql .= " LIMIT " . (($sPage - 1) * PAGE_MAX) . ", " . PAGE_MAX; //表示の設定変更
	}
...
function fnSqlArticleEdit($articleNo)
{
	...
	$sql .= " FROM TBLARTICLE";
	$sql .= " WHERE ARTICLENO = $articleNo"; //値を変更
